            ...
            if block_rate >= self.block_rate_threshold:
                self.alerts.append(
                    Alert(
                        ...
                        message=f"High block rate detected: {block_rate:.2f} >= {self.block_rate_threshold}",
                    )
                )
        ...
        if self.rate_limit_hits >= self.rate_limit_hit_threshold:
            self.alerts.append(
                Alert(
                    ...
                    message=f"High rate limit hits: {self.rate_limit_hits} >= {self.rate_limit_hit_threshold}",
                )
            )
            ...
            if judge_fail_rate >= self.judge_fail_rate_threshold:
                self.alerts.append(
                    Alert(
                        ...
                        message=f"High judge failure rate: {judge_fail_rate:.2f} >= {self.judge_fail_rate_threshold}",
                    )
                )
